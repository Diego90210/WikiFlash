...
- **Dependencies:** 1.2
- **Tasks:**
  - [x] Create `components/WikipediaInput.tsx` (integrated into CreateDeckView)
  - [x] Build input field with shadcn Input component
  - [x] Add placeholder: "Enter topic or paste Wikipedia URL"
  - [x] Add submit button
  - [x] Style with custom colors
...
- **Dependencies:** 2.1
- **Tasks:**
  - [x] Create `lib/wikipedia/detectInputType.ts`
  - [x] Write regex to detect Wikipedia URLs
  - [x] Return type: 'url' or 'topic'
  - [x] Handle edge cases (mobile URLs, different language Wikipedias)
  - [ ] Write unit tests
...
- **Dependencies:** 2.2
- **Tasks:**
  - [x] Create `lib/wikipedia/api.ts`
  - [x] Implement `searchWikipedia(topic)` function using Wikipedia API
  - [x] Implement `getPageContent(title)` function
  - [x] Handle URL parsing to extract page title
  - [x] Handle disambiguation pages (return suggestions)
...
- **Dependencies:** 2.3
- **Tasks:**
  - [x] Create `lib/wikipedia/parser.ts`
  - [x] Extract plain text from Wikipedia HTML
  - [x] Remove references, citations, navigation elements
  - [x] Preserve paragraph structure
  - [x] Limit content to first 5000 words (to avoid token limits)
  - [x] Extract article title and summary
...
- **Tasks:**
  - [x] Add loading spinner to input component
  - [x] Handle "page not found" errors
  - [x] Handle network errors
  - [x] Show error messages with shadcn Toast
...
- **Dependencies:** 2.1
- **Tasks:**
  - [x] Add number input or slider component
  - [x] Set range: 5-50 cards
  - [x] Default value: 20 cards
  - [x] Style to match design
  - [x] Pass value to flashcard generation function
...
- **Dependencies:** None
- **Tasks:**
  - [x] Sign up for Groq API at console.groq.com
  - [x] Get API key
  - [x] Add `GROQ_API_KEY` to `.env.local`
  - [x] Install Groq SDK or use fetch directly
...
- **Tasks:**
  ...
  - [x] Design system prompt for flashcard generation
  - [x] Include instructions for: definitions, examples, facts, dates, causes/effects
  - [x] Specify JSON output format: `{question: string, answer: string}[]`
  - [x] Add examples of good flashcards
...
- **Tasks:**
  - [x] Create `lib/ai/generateFlashcards.ts`
  - [x] Build function: `generateFlashcards(content, count, topic)`
  - [x] Make API call to Groq with prompt
  - [x] Use model: `llama-3.3-70b-versatile`
  - [x] Parse JSON response
  - [x] Validate flashcard format
  - [x] Handle parsing errors
  - [x] Return array of flashcards
...
- **Dependencies:** None
- **Tasks:**
  - [x] Find or create shuffling hands animation (Lottie or CSS)
  - [x] Option 2: Create CSS keyframe animation
  - [x] Create `components/GeneratingView.tsx` (with animation)
  - [x] Add loading text: "Generating X flashcards..."
  - [x] Add cancel button
  - [x] Style to match design
...
- **Dependencies:** 3.3
- **Tasks:**
  - [x] Create `components/CardPreviewView.tsx`
  - [x] Display generated cards in grid layout
  - [x] Show question and answer for each card
  - [x] Add edit button per card (opens dialog)
  - [x] Add delete button per card
  - [x] Create edit dialog with form
  - [x] Handle card updates in state
  - [x] Add "Save Deck" button at bottom
  - [x] Add "Regenerate" button option
...
- **Tasks:**
  - [x] Create `lib/supabase/decks.ts`
  - [x] Implement `createDeck(name, topic, url, sessionId)`
  - [x] Implement `createCards(deckId, cards[])`
  - [x] Use Supabase transactions for atomicity
  - [x] Handle errors (show alert on failure)
...
- **Dependencies:** None
- **Tasks:**
  - [x] Create `lib/spaced-repetition/sm2.ts`
  - [x] Implement `updateCard(card, quality)` function
  - [x] Quality mapping: Hard=0, Good=1, Easy=2
  - [x] Calculate new ease factor
  - [x] Calculate new interval
  - [x] Calculate next review date
  - [x] Ensure ease factor stays >= 1.3
...
- **Dependencies:** 1.4
- **Tasks:**
  - [x] Create `lib/supabase/cards.ts`
  - [x] Implement `getDueCards(deckId)` query
  - [x] Filter cards where `next_review <= today`
  - [x] Order by: due cards first, then new cards
  - [x] Return cards with all SM-2 data
...
- **Dependencies:** 4.2
- **Tasks:**
  - [x] Create `components/StudySession.tsx` (integrated in StudySessionView)
  - [x] Fetch due cards on mount
  - [x] Handle "no cards due" state
  - [x] Initialize session state (current card index, cards reviewed)
  - [x] Render current card
  - [x] Handle session end (show summary)
...
- **Dependencies:** 4.3
- **Tasks:**
  - [x] Create `components/Flashcard.tsx` (integrated in StudySessionView)
  - [x] Show question side initially
  - [x] Add "Show Answer" button
  - [x] Implement card flip animation (CSS 3D transform)
  - [x] Show answer side after flip
  - [x] Style card with surface color
  - [x] Make card large and centered
...
- **Dependencies:** 4.4
- **Tasks:**
  - [x] Create `components/RatingButtons.tsx` (integrated in StudySessionView)
  - [x] Create 3 buttons: Hard, Good, Easy
  - [x] Style with different colors (red, yellow, green variants)
  - [x] Disable buttons until answer is revealed
  - [x] Handle click events
  - [x] Pass rating to parent component
...
- **Tasks:**
  - [x] In StudySession, handle rating submission
  - [x] Call SM-2 algorithm with current card and rating
  - [x] Get updated card data
  - [x] Save updated card to Supabase
  - [x] Move to next card
  - [x] Update session progress
  - [x] Handle last card (end session)
...
- **Dependencies:** 4.3
- **Tasks:**
  - [x] Create `components/ProgressBar.tsx` (integrated in StudySessionView)
  - [x] Show cards completed / total cards
  - [x] Display as bar at top of study session
  - [x] Update in real-time as cards are reviewed
  - [x] Style with primary color
...
- **Dependencies:** 4.6
- **Tasks:**
  - [x] Create `components/SessionSummary.tsx` (SessionCompleteView)
  - [x] Show cards reviewed count
  - [x] Show breakdown: Hard/Good/Easy ratings
  - [x] Calculate and show accuracy or success rate
  - [x] Add "Back to Dashboard" button
  - [x] Add "Study Again" button (if more cards available)
  - [x] Style as a card modal
...
- **Tasks:**
  ...
  - [x] Update deck's `last_studied_at` timestamp
  - [x] Handle errors silently (don't block user)
...
- **Tasks:**
  ...
  - [x] Ensure all errors show user-friendly messages
  - [x] Add error boundaries for React errors
